...
export class ImageService {
  ...
  /** 图片落盘目标目录：文档同目录 assets；未保存文档为 null（主进程用默认目录） */
  private assetDir(): string | null {
    const docPath = this.fileService.getFilePath()
    return docPath ? `${dirnamePath(docPath)}/assets` : null
  }
  ...
  async insertFromUrl(url: string): Promise<void> {
    const result = await this.api.downloadImage({ url, destDir: this.assetDir() })
