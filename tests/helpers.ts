...
export async function loadContent(
  ...
  await handle.app.evaluate(({ BrowserWindow }, payload) => {
...
export function sendCommand(
  ...
  return handle.app.evaluate(({ BrowserWindow }, c) => {
...
/** 渲染层 window.prompt 打桩（插入链接 / 图片命令用），可返回多个答案 */
export function stubPrompts(handle: AppHandle, answers: string[]): Promise<void> {
  return handle.window.evaluate((props) => {
    let i = 0
    window.prompt = () => props[Math.min(i++, props.length - 1)]
  }, answers)
}