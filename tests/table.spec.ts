...
async function clickCell(row: number, col: number): Promise<void> {
  await app.window.evaluate(({ r, c }) => {
    const cell = document
      .querySelectorAll('.table-scroll-wrapper table tbody tr')[r]
      ?.querySelectorAll('td, th')[c] as HTMLElement | undefined
    if (!cell) return
...
function headerCellCount(): Promise<number> {
  return app.window
    .locator('.table-scroll-wrapper table tr')
    .first()
    .locator('th, td')
    .count()
}
...
/** 载入文档的便捷包装（与 helper 保持一致，内部使用 loadContent） */
function loadDocument(_app: AppHandle, content: string): Promise<void> {
  return loadContent(app, content)
}