...
function callDownload(url: string): Promise<SaveResult> {
  return app.window.evaluate(
    async ({ url, docPath }) => {
      return (
        window as unknown as { typewren: { downloadImage: (p: unknown) => Promise<SaveResult> } }
      ).typewren.downloadImage({ url, docPath });
    },
    { url, docPath: join(destDir, 'doc.md') }
  );
}
...
test.describe('剪贴板位图（base64）落盘', () => {
  test('合法 PNG 落盘', async () => {
    const res = await app.window.evaluate(
      async ({ base64, docPath }) => {
        return (
          window as unknown as {
            typewren: { saveImageFromData: (p: unknown) => Promise<SaveResult> };
          }
        ).typewren.saveImageFromData({ base64, mime: 'image/png', docPath });
      },
      { base64: PNG_BASE64, docPath: join(destDir, 'doc.md') }
    );
    expect(res.ok).toBe(true);
    const savedPath = res.savedPath!;
    expect(existsSync(savedPath)).toBe(true);
  ...
  test('空数据被拒绝', async () => {
    const res = await app.window.evaluate(
      async ({ dest }) => {
        ...
        ).typewren.saveImageFromData({ base64: '', mime: 'image/png', docPath: dest });
      },
      { dest: join(destDir, 'doc.md') }
    );
    expect(res.ok).toBe(false);
  });
  ...
  test('超过 25MB 的 base64 被拒绝', async () => {
    ...
    const res = await app.window.evaluate(
      async ({ base64, docPath }) => {
        ...
        ).typewren.saveImageFromData({ base64, mime: 'image/bmp', docPath });
      },
      { base64: big64, docPath: join(destDir, 'doc.md') }
    );
    expect(res.ok).toBe(false);
  });
});
...
test.describe('未保存文档图片落盘到用户数据区', () => {
  test('docPath 为 null 时写入 userData/images', async () => {
    const res = await app.window.evaluate(
      async ({ base64 }) => {
        return (
          window as unknown as {
        ...
        ).typewren.saveImageFromData({ base64, mime: 'image/png', docPath: null });
      },
      { base64: PNG_BASE64 }
    );
    expect(res.ok).toBe(true);
