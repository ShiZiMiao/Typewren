...
/** 报告缓存：完整 remark 管线较重（真实文件 + 全用例），beforeAll 跑一次各用例复用 */
let report: CheckReport = { failures: [], report: [] };
...
test('删除线仍序列化为 ~~ 且 AST 是 delete 节点', () => {
  expect(pick('strike').out).toBe('~~删除~~');
  expect(pick('strike-ast').pass).toBe(true);
});
